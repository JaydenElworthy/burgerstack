...
export default function BurgerGame() {
  ...
  // 1. Define async storage functions first to avoid hoisting/stale state references
          ...
          <AnimatePresence>
            {!isExiting && (
              <motion.div
                ...
                exit={{ x: "250%", transition: { duration: 0.4, ease: "expoIn" } }}
                transition={{ x: { type: "tween", ease: "circOut", duration: 0.5 } }}
                // RAISED: bottom-[80px] on mobile to give it more lift
                className="absolute bottom-[80px] md:bottom-[100px] left-1/2 w-64 md:w-80 h-[300px] z-30 pointer-events-none"
              >
                {stack.map((item, i) => {
                  let elev = 0;
          ...
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full max-w-sm font-sans">
            {gameState === 'won' ? (
              <div className="mb-8">
                ...
                <p className="text-lg font-bold uppercase text-white tracking-widest">Picnic Ready!</p>
              </div>
            ) : gameState === 'lost' ? (
              <div className="mb-8">
                ...
                <p className="text-lg font-bold uppercase text-white opacity-80 uppercase tracking-widest">Kitchen Exploded!</p>
              </div>
            ) : (
            ...
            <div className="flex flex-col gap-4">
              ...
              <h2> How To Play </h2>
              <h3 className="text-lg font-bold uppercase text-white opacity-80 uppercase tracking-widest">Add your Bun => Add your Patty => add your Cheese => Add your Bun</h3>
              {gameState !== 'start' && (
