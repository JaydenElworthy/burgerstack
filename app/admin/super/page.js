...
const handleBulkUpload = async () => {
  ...
  const { error } = await supabase.from('prize_pool').insert(
    codeList.map(code => ({ code: code.trim(), is_claimed: false }))
  );

  if (error) {
    alert("Error: Some codes might already exist in the database.");
  } else {
    alert(`Successfully added ${codeList.length} codes to the pool!`);
    setRawCodes('');
  }
};
